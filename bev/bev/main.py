...
expr_dir = '/home/groups/syyeung/behavioral_coding/V02_05'
...
class BEV(nn.Module):
    ...
    def process_normal_image(self, image, signal_ID):
        ...
        if self.settings.calc_smpl:
            if self.settings.pose_aug:
                thetas = outputs['smpl_thetas']
                thetas_z = vp.encode(thetas[:, 3:-6]).rsample()
                thetas_z = thetas_z * (1 + torch.randn_like(thetas_z, device=thetas_z.device))
                aug_thetas = vp.decode(thetas_z)
                thetas[:, 3:-6] = aug_thetas['pose_body'].reshape(-1, 63)

                # Augment orientation.
                thetas[:, :3] = thetas[:, :3] * (1 + 0.1 * torch.randn_like(thetas[:, :3], device=thetas.device))

                outputs['smpl_thetas'] = thetas
                outputs['smpl_betas'] = outputs['smpl_betas'] * (1 + 0.1 * torch.randn_like(outputs['smpl_betas'], device=thetas.device))
                # move the person to the center of the image, and closer to the camera,
                # so that the depth map is not too small.
                outputs['cam'] = torch.tensor([[1.2, 0, 0]]).to(outputs['cam'].device)
                outputs.update({'cam_trans': denormalize_cam_params_to_trans(outputs['cam'])})  # [0., 0., 1.4413]

                # only keep the first person in the outputs.
                for key in outputs:
                    outputs.update({key: outputs[key][:1]})

            # print('Hack!!! Change to fat people')
            # fake_betas = torch.zeros_like(outputs['smpl_betas']).to(outputs['smpl_betas'].device)
            # import pdb; pdb.set_trace()
            # fake_betas[:, :10] -= 4
            # outputs['smpl_betas'] = fake_betas
