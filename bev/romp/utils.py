...
class ResultSaver:
    def __init__(self, mode='image', save_path=None, save_npz_path=None, save_npz=True):
    ...
    def __call__(self, outputs, input_path, prefix=None, img_ext='.png'):
        if self.mode == 'video' or self.is_dir:
            ...
            save_npz_path = osp.join(self.save_npz_path, osp.splitext(save_name)[0])+'.npz'
        ...
        if outputs is not None:
            ...
            if self.save_npz:
                np.savez(save_npz_path, results=outputs)

            cv2.imwrite(save_path, rendered_image)    
        else:
            print(f'No detections for image {input_path}.')
