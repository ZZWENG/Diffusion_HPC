...
def agora_train_extract(dataset_path, out_path):
    dataset_path = os.path.join(dataset_path, 'images_highRes')
    ...
    for split in tqdm.tqdm(train_split):
        images_path = os.path.join(dataset_path, 'train_{}'.format(split))
        keypoints_path = os.path.join(dataset_path, 'keypoints_{}'.format(split))
        images = glob.glob(os.path.join(images_path, '*.png')) 
        print('Split', split, ':', len(images), 'images')
    ...
    out_file = os.path.join(out_path, 'agora_train_keypoints.npz'.format(split))
