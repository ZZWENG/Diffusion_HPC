...
class BaseDataset(Dataset):
    ...
    def __init__(self, options, dataset, ignore_3d=False, use_augmentation=True, is_train=True, parse_random_bg=False):
        ...
        self.img_dir = config.DATASET_FOLDERS.get(dataset, '')
        self.normalize_img = Normalize(mean=constants.IMG_NORM_MEAN, std=constants.IMG_NORM_STD)
        if is_train:
            self.data = np.load(join(config.DATASET_NPZ_PATH, f'{dataset}_train.npz'))
        else:
            self.data = np.load(join(config.DATASET_NPZ_PATH, f'{dataset}_test.npz'))
        ...
        try:
            ...
            self.betas = self.data['shape'].astype(np.float)[:, :10]
