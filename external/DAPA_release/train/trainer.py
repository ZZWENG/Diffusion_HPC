...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        if freeze_backbone:
            ...
            self.model.decpose.weight.requires_grad = True
            self.model.decshape.weight.requires_grad = True
            self.model.deccam.weight.requires_grad = True
        else:
            for param in self.model.parameters():
                param.requires_grad = True
        params_to_grad = [p for p in self.model.parameters() if p.requires_grad]
        self.optimizer = torch.optim.Adam(params=params_to_grad,
                                          lr=self.options.lr,
                                          weight_decay=0)
...
#                 return torch.FloatTensor(1).fill_(0.).to(self.device)
    ...
    def smpl_losses(self, pred_rotmat, pred_betas, gt_pose, gt_betas, has_smpl, reduce=True):
        ...
        if len(pred_rotmat_valid) > 0:
            if reduce:
                loss_regr_pose = self.criterion_regr(pred_rotmat_valid[:, 1:], gt_rotmat_valid[:, 1:])
                loss_regr_betas = self.criterion_regr(pred_betas_valid[:, 1:], gt_betas_valid[:, 1:])
            else:
                loss_regr_pose = self.criterion_regr_nr(pred_rotmat_valid[:, 1:], gt_rotmat_valid[:, 1:]).mean((1,2,3))
                loss_regr_betas = self.criterion_regr_nr(pred_betas_valid[:, 1:], gt_betas_valid[:, 1:]).mean(1)
        else:
            if reduce:
