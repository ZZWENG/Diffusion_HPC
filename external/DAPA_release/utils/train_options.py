...
class TrainOptions():
    ...
    def __init__(self):
        ...
        train.add_argument('--test_steps', type=int, default=4000, help='Testing frequency during training')
        ...
        train.add_argument('--scale_factor', type=float, default=0.0, help='Rescale bounding boxes by a factor of [1-scale_factor,1+scale_factor]') 
#         train.add_argument('--no_edge_padding', default=False, action='store_true', help='whether to do repeated edge padding (seedlings)') # TODO: maybe don't do this for AGORA.
        ...
        # new flags
        train.add_argument('--pretrain', default=False, action='store_true', help='Run pre-training')
        train.add_argument('--use_opt_global_orient', default=False, action='store_true')
        train.add_argument('--train_texture', default=False, action='store_true')
        train.add_argument('--donot_train_on_fake', default=False, action='store_true')
        train.add_argument('--generator_type', type=str, default='default', choices=['default', 'hidden'])
        train.add_argument('--g_hid_dim', type=int, default=50)
        train.add_argument('--generator_kl_weight', type=float, default=0.)
        train.add_argument('--g_learn_mean_pose', default=False, action='store_true')
        train.add_argument('--g_train_prior', default=False, action='store_true')
        
        # options for adaptation
        train.add_argument('--wandb_project', default='DAPA', help="Wandb project name.")
        # train.add_argument('--seedlings', default=False, action='store_true')
        # train.add_argument('--agora', default=False, action='store_true')
        train.add_argument('--ft_dataset', default='', help="Name of the target dataset.")
        
        train.add_argument('--adapt_baseline', default=False, action='store_true', help='Baseline SPIN model. Not using synthetic images at all.')
        train.add_argument('--use_texture', default=False, action='store_true')
        train.add_argument('--add_background', default=False, action='store_true')
        train.add_argument('--g_input_noise_scale', type=float, default=0.05)
        train.add_argument('--surreal_texture', default=False, action='store_true')
        # train.add_argument('--g_n_iter', type=int, default=3)
        train.add_argument('--g_input_noise_type', type=str, default='randn', choices=['randn', 'fgsm', 'mul', 'rand_sample'])
        train.add_argument('--vposer', default=False, action='store_true')
    ...
    def parse_args(self):
        ...
        if self.args.from_json is not None:
            ...
            with open(path_to_json, "r") as f:
                ...
                json_args['cmdline_checkpoint'] = self.args.checkpoint
                for k in vars(self.args):
                    if k not in json_args:
                        json_args[k] = vars(self.args)[k]
